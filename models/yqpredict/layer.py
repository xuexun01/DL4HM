...
class StructuralAttentionLayer(nn.Module):
    def __init__(self, input_dim, output_dim, n_heads, attn_drop, ffd_drop, residual):
        ...
        self.linear = nn.Linear(input_dim, n_heads * self.out_dim, bias=False)
    ...
    def forward(self, graph):
        ...
        x = self.linear(graph.x).view(-1, H, C) # [N, heads, out_dim]
        # attention
        ...
        # output
        out = self.activate(scatter(x_j * coefficients[:, :, None], edge_index[1], dim=0, reduce="sum"))
